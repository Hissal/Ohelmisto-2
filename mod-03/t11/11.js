...
// add your code here
function constructArticle(entry) {
  ...
  img.src = entry.image.medium;
  img.alt = entry.title;
  figure.appendChild(img);
  const figcaption = document.createElement('figcaption');
  figcaption.innerText = entry.caption;
  figure.appendChild(figcaption);
  article.appendChild(figure);

  const paragraph = document.createElement('p');
  paragraph.innerText = entry.description;
  article.appendChild(paragraph);

  return article;
}

const articles = picArray.map(constructArticle);
const largeImages = picArray.map((entry) => entry.image.large);

const dialog = document.querySelector('dialog');
const dialogImg = dialog.querySelector('img');
const picSection = document.getElementById('pictures');
...
picSection.addEventListener('click', (event) => {
  ...
  const articleElement = clickedElement.tagName === 'ARTICLE' ? clickedElement : clickedElement.closest('article');
  const articleIndex = articles.indexOf(articleElement);
  if (articleIndex !== -1) {
    dialogImg.src = largeImages[articleIndex];
    dialogImg.alt = articleElement.querySelector('h2').innerText;
    dialog.showModal();
  }
});
